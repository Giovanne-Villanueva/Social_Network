...
module.exports = {
  ...
  async createThought(req, res) {
    try {
      ...
      if(!user){
        return res.status(404).json({ message: 'Failed to update user\'s associated thought'});
      }
      res.json(thought);
    } catch (err) {
  ...
  // Delete a thought and remove them from any users
    ...
    try {
      ...
      if (!user) {
        return res.status(404).json({
          message: 'Thought deleted, but no users found associated with thought',
        });
      }
